...
## 8. Celery
Airflow는 기본적으로 sqlite를 이용하며, sqlite는 SequentialExecutor만 설정가능하기에,   
DAG 내에서 Task의 병렬 실행이 불가능하다. 병렬 실행을 위해 CeleryExecutor를 사용해야하는데   
그러기 위해선 sqlite가 아닌 Redis나 RabbitMQ와 같은 message broker를 사용해야 한다.  
